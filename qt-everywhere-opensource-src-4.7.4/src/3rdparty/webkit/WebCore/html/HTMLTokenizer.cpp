...
// SSLAB : HTML 파싱 및 태그 복원 함수.
void HTMLTokenizer::writet(const SegmentedString& str, bool appendData)
{
    if (!m_buffer)
        return;
...
*/
    ...
    printf("HTMLTokeinizer:writet : Restoring tags of a HTML piece.\n");
    while(m_parser->cnt > -1) { n = m_parser->parseToken(&m_currentToken); }
