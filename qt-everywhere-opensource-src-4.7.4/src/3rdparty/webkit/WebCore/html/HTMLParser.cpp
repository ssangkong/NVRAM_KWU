...
void HTMLParser::printTokenBuilderV2(Token* t) {
...
/*
    sprintf(path, "%u", RSHash(m_document->url().string().impl()->ascii().data(), m_document->url().string().impl()->length()));
    if(access(path, 0)) {
    mkdir(path, 0777); 
}

    sprintf(path, "%u/%u", RSHash(m_document->url().string().impl()->ascii().data(), m_document->url().string().impl()->length()), domHashVal);
    if(access(path, 0)) {
    mkdir(path, 0777);
}

    sprintf(path, "%u/%u/%d.bin", RSHash(m_document->url().string().impl()->ascii().data(), m_document->url().string().impl()->length()), domHashVal, cnt++);
    FILE *fp = fopen(path, "ab");
*/
// for experimental


    sprintf(path, "/home/sslab/ramdisk/dom");//, RSHash(m_document->url().string().impl()->ascii().data(), m_document->url().string().impl()->length()));
    if(access(path, 0)) mkdir(path, 0777);

    sprintf(path, "/home/sslab/ramdisk/dom/%u", RSHash(m_document->url().string().impl()->ascii().data(), m_document->url().string().impl()->length()));
    if(access(path, 0)) mkdir(path, 0777);

    sprintf(path, "/home/sslab/ramdisk/dom/%u/%d.bin", RSHash(m_document->url().string().impl()->ascii().data(), m_document->url().string().impl()->length()), cnt++);
    FILE *fp = fopen(path, "ab");

//============

    // delimiter
    fwrite(delimiter, sizeof(unsigned char), 2, fp);

    // # of attributes
    if(t->attrs) flag = (unsigned char)t->attrs->length();
    flag = flag & 0x0f;

    // flags 
    if(t->beginTag)		flag = flag | 0x80;
    if(t->selfClosingTag)	flag = flag | 0x40;
    if(t->brokenXMLStyle)	flag = flag | 0x20;
    if(t->tagName == textAtom)	flag = flag | 0x10;
...
//============
    ...
    // TagName's length
    fwrite(&(buf = t->tagName.string().length()), sizeof(unsigned int), 1, fp);

    // TagName
    fwrite(t->tagName.string().impl()->ascii().data(), sizeof(unsigned char), t->tagName.string().length(), fp);
//  printf("%s %d %d\n",t->tagName.string().impl()->ascii().data(), sizeof(unsigned char), t->tagName.string().length());
...
//============
    ...
    if(t->attrs) {
        int i, j = t->attrs->length();                                                        
        for(i = 0; i < j; ++i) {                               
             target = t->attrs->attributeItem(i);                                             
             // delimiter
             fwrite(delimiter, sizeof(unsigned char), 2, fp);

             // Length of attribute's name
             fwrite(&(buf = target->localName().length()), sizeof(unsigned int), 1, fp);

             // Attribute's name
	     fwrite(target->localName().string().impl()->ascii().data(), 1, target->localName().length(), fp);

             // delimiter
             fwrite(delimiter, sizeof(unsigned char), 2, fp);

             // Length of attribute's value
             fwrite(&(buf = target->value().length()), sizeof(unsigned int), 1, fp);

             // Attribute's value
             fwrite(target->value().string().impl()->ascii().data(), 1, target->value().length(), fp);
        }
    }    
...
//============
    ...
    if(t->text.get())
        ...
        // Text's length
        fwrite(&(buf = text.length()), sizeof(unsigned int), 1, fp);

        // Text
        fwrite(text.impl()->ascii().data(), 1, text.length(), fp);
    }
...
/*
    ...
    printf("else if(cnt == %d) { t = new Token(); \n", cnt);
    printf("    t->tagName = AtomicString(\"%s\"); \n", t->tagName.string().impl()->ascii().data());
    printf("    t->beginTag = %d; \n", t->beginTag);
    printf("    t->selfClosingTag = %d; \n", t->selfClosingTag);
    printf("    t->brokenXMLStyle = %d; \n", t->brokenXMLStyle);
    ...
    if(t->attrs) {
        ...
        for(i = 0; i < j; ++i) {
             target = t->attrs->attributeItem(i);
             printf("    AtomicString n%d(\"%s\"); \n", i, target->localName().impl()->ascii().data());
             printf("    AtomicString v%d(\"%s\"); \n", i, target->value().impl()->ascii().data());
             printf("    t->addAttribute(n%d, v%d, true); \n", i, i);
        }
    }
    ...
    if (t->tagName == textAtom && t->text && m_current->localName() != scriptTag) {
        String text = t->text.get();
    if(t->text.get())
        printf("    t->text = StringImpl::create(\"%s\"); \n", text.impl()->ascii().data());
    }
...
Token* HTMLParser::rebuildToken(Token* t)
{
    char delimiter[2];
    char path[100];
    char cBuffer[2048] = {0,};
...
/*beginTag)             flag = flag | 0x80;
    if(t->selfClosingTag)       flag = flag | 0x40;
    if(t->brokenXMLStyle)       flag = flag | 0x20;
    if(t->tagName == textAtom)  fl
*/

    bool beginTag = false, selfClosingTag = false, brokenXMLStyle = false, bTextAtom = false;
    unsigned int j, idx, len, hdr;

    FILE *fp = NULL;
    unsigned int nByteOfRead = 0;
...
//    sprintf(path, "%u/%u/%d.bin", RSHash(m_document->url().string().impl()->ascii().data(), m_document->url().string().impl()->length()), domHashVal, cnt++);
//    printf("HTMLParser::rebuildToken : Restoring a tag. %s\n", path);

// for experimental



//    sprintf(path, "test/%d.bin", cnt++);
    sprintf(path, "/home/sslab/ramdisk/dom/%u/%d.bin", RSHash(m_document->url().string().impl()->ascii().data(), m_document->url().string().impl()->length()), cnt++);

    if(!(fp = fopen(path, "rb"))) { cnt = -1; return NULL; }

    while(0 < (nByteOfRead = fread(cBuffer, sizeof(unsigned char), 2048, fp))) {
        cBuffer[nByteOfRead] = '/0';
        buf = new String(cBuffer, nByteOfRead < 2048 ? nByteOfRead : 2048);
        str.append(*buf);
        delete buf;

//        str.append((char*)cBuffer, nByteOfRead < 2048 ? nByteOfRead : 2048);
    }
...
*/
    ...
    if(flag & 0x80) beginTag = true;
    if(flag & 0x40) selfClosingTag = true;
    if(flag & 0x20) brokenXMLStyle = true;
    if(flag & 0x10) bTextAtom = true;
    
    j = (unsigned int)(flag & 0x0f);

    t->beginTag = beginTag; 
    t->selfClosingTag = selfClosingTag; 
    t->brokenXMLStyle = brokenXMLStyle; 

    // TagName
    hdr = 5;
    len = 0 | str[hdr+0] | str[hdr+1] << 2 | str[hdr+2] << 4 | str[hdr+3] << 6;
    idx = 9;    

//    printf("tagName : %s %d\n", str.substring(idx, len).impl()->ascii().data(), cnt-1);
    t->tagName = AtomicString(str.substring(idx, len).impl());
...
//        printf("Attribute's Name : %s\n", str.substring(idx, len).impl()->ascii().data());
        n = AtomicString(str.substring(idx, len).impl());
...
//        printf("Attribute's Value : %s\n", str.substring(idx, len).impl()->ascii().data()); 
        v = AtomicString(str.substring(idx, len).impl()); 

        t->addAttribute(n, v, true);
    }   
...
//        printf("Text : %s\n", str.substring(idx, len).impl()->ascii().data());
        t->text = str.substring(idx, len).impl();
    }
...
/*    
    ...
    buf = &(str.substring(5, 4));
    //len = 0 | b[0] | b[1] << 2 | b[2] << 4 | b[3] << 6;


    
    printString((str.substring(9,20)));

    printf("tagName : %s\n", str.substring(9, len).impl()->ascii().data());
    ...
    for(int i = 0; i < j; i++) {
        ...
        const String &b = str.substring(5, 4);
        len = 0 | b[0] | b[1] << 2 | b[2] << 4 | b[3] << 6;
...
unsigned int HTMLParser::RSHash(const char* str, unsigned int len)
{
...
// SSLAB : Restoring DOM from HTMLTokenizer::writet().
//    if(cnt >= 0) {
        if(!(t = rebuildToken(tt))) { return NULL; }
//    }
//    else
//        printTokenBuilderV2(t);
