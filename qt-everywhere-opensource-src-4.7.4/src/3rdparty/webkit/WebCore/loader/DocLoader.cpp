...
// SSLAB : Include additional headers.
#include <QDateTime>
...
// SSLAB
void DocLoader::setBoolLoad()
{
    cache()->setBoolLoad();
}

// SSLAB
void DocLoader::loadable()
{
    cache()->loadable();
}

// SSLAB
void DocLoader::debugging()
{
    cache()->debugging();
}

// SSLAB
void DocLoader::pCaching()
{
    cache()->pCaching();
}

// SSLAB
void DocLoader::tracing()
{
    cache()->tracing();
}
...
// SSLAB : Initialize flags for checking whether the resource is cached or valid.
    cache()->setValid(true);
    ...
    // SSLAB : If the resource is not cached, 
    CachedResource* existing = cache()->resourceForURL(fullURL.string());
    if (!existing || existing->isPreloaded()) {
        cache()->setCached(false);
        printf("SSLAB : DocLoader : checkForReload : Not cached!!!\n");
        return;
    }

    // SSLAB : If the resource is cached, check validity of the resource.
    cache()->setCached(true);

    switch (cachePolicy()) {
    case CachePolicyVerify:
        printf("SSLAB : DocLoader : checkForReload : case CachePolicyVerify.\n");
        if (!existing->mustRevalidate(CachePolicyVerify)) {
            printf("sslab : DocLoader : checkForReload : Cached & Valid!!!\n");
            cache()->setValid(true);
            return;
        }
        printf("SSLAB : DocLoader : checkForReload : Cache & Invalid!!! (Call revalidateResource)\n");
        cache()->setValid(false);
        cache()->revalidateResource(existing, this);
        break;
    case CachePolicyCache:
        printf("SSLAB : DocLoader : checkForReload : case CachePolicyCache.\n");
        if (!existing->mustRevalidate(CachePolicyCache)) {
            printf("SSLAB : DocLoader : checkForReload : Cached & Valid!!!");
            cache()->setValid(true);
            return;
        }
        printf("SSLAB : DocLoader : checkForReload : Cached & Invalid!!! (Call revalidateResource)\n");
        cache()->setValid(false);
        cache()->revalidateResource(existing, this);
        break;
    // SSLAB : For load type "no-cache", regard the resource's load type as "not cached".
    case CachePolicyReload: // for "Cache-control : no-cache"
        printf("SSLAB : DocLoader : checkForReload : CachePolicyReload.\n");
        printf("SSLAB : DocLoader : checkForReload : Not cached!!! (call remove)\n");
        cache()->setCached(false);
        cache()->remove(existing);        
        break;
    case CachePolicyRevalidate:
        printf("SSLAB : DocLoader : checkForReload : CachePolicyRevalidate.\n");
        printf("SSLAB : DocLoader : checkForReload : Cached & Invalid!!! (call revalidateResource)\n");
        cache()->setValid(false);
        cache()->revalidateResource(existing, this);
        break;
    case CachePolicyAllowStale:
        printf("SSLAB : DocLoader : checkForReload : CachePolicyAllowStale.\n");
        printf("SSLAB : DocLoader : checkForReload : Cached & Valid!!!\n");
        cache()->setValid(true);
        return;
    }
...
CachedResource* DocLoader::requestResource(CachedResource::Type type, const String& url, const String& charset, bool isPreload)
{
    ...
    // SSLAB: Request what?
    printf("\nSSLAB : DocLoader : requestResource : requesting... %s\n", url.utf8().data());

    // SSLAB : Start point of the loading the resource.
    ...
    // SSLAB : Toggle btw enable or disable of sub-resource loading.
    if(cache()->boolLoadable) {
        printf("SSLAB : DocLoader : requestResource : === start 1st validation checking... \n");
        checkForReload(fullURL);
        printf("SSLAB : DocLoader : requestResource : === end 1st validation checking ...\n");
    }
    ...
    // SSLAB : End point of the loading the resource.
    currentTime = QDateTime::currentDateTime();
    qint64 endTime = currentTime.currentMSecsSinceEpoch();

    // SSLAY : Stamp load type of the resource.
    cache()->loadStamp((double)(endTime)-(double)(startTime), url);
