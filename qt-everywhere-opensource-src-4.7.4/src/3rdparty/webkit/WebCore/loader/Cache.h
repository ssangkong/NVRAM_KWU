...
#include <CString.h>
// SSLAB : Path of NVRAM.
#define NVRAM_PATH "/home/sslab/ramdisk/"

namespace WebCore  {

// SSLAB : Unit of caching.
static const unsigned segmentSize = 0x10000;
...
enum Status {
    ...
    // SSLAB : Toggle funtions.
    void setBoolLoad() { printf("SSLAB / Loading a page...\n"); 
			 bl = true; 
    //                     prune();
		       }

    void loadable() { if(boolLoadable) { boolLoadable = false; printf("SSLAB : Disabled sub-resource loading.\n"); } 
                      else { boolLoadable = true; printf("SSLAB : Enabled sub-resource loading.\n"); }
    }
    ...
    void debugging() { 
		      ...
		      if(boolDebugging) { boolDebugging = false; printf("SSLAB : Disabled debug messaging.\n"); } 
                      else { boolDebugging = true; printf("SSLAB : Enabled debug messaging.\n"); }
    }

    void pCaching() { if(boolPCaching) { boolPCaching = false; printf("SSLAB : Disabled p_cache.\n"); }
                      else { boolPCaching = true; printf("SSLAB : Enabled p_cache.\n"); }
    }

    void tracing() { if(boolTracing) { boolTracing = false; printf("SSLAB : Disabled sub-resources tracing.\n"); }
                      else { boolTracing = true; printf("SSLAB : Enabled sub-resources tracing.\n"); }
    }

    void loadStamp(double loadtime, const String& url);
    void initLoadStamp();
    void customStats();  
    void dumpAgain(CachedResource*);
    void dumpContents(CachedResource*);
    void restoreContents(CachedResource::Type, const char*, const char*, const char*);
    CachedResource* restoreContentsV2(CachedResource::Type, const char*, const char*);
    unsigned int RSHash (const char* str, unsigned int len);

    // SSLAB : Signal from browserMainWindow.
    bool bl;    
    bool boolLoadable;
    bool boolDebugging;
    bool boolPCaching;
    ...
    // SSLAB : Flags for checking whether the resource loading type.
    void setValid(bool flag) { valid = flag; }
    void setCached(bool flag) { cached = flag; }
